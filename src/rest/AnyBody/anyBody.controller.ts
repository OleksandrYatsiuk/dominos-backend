...
export default class AnyBodyController extends Controller {
	...
	private registering = async (request: express.Request, response: express.Response, next: express.NextFunction) => {
		...
		if (registerData.password !== registerData.confirmPassword) {
			...
			this.helper
				.createUser(
					Object.assign(registerData, { passwordHash: this.helper.createPasswordHash(registerData.password) })
				)
				.then((user) =>
					this.accessToken
	...
	private confirm = (request: express.Request, response: express.Response, next: express.NextFunction) => {
		...
		this.accessToken.checkAccessTokenValid(token).then((result) => {
			switch (result) {
				...
				case true:
					return this.accessToken.confirmEmail(token).then((res) => code200(response, null));
			}
		});
	};
